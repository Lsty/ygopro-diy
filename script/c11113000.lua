...
function c11113000.matfilter(c)
	return c:GetLevel()>0 and c:IsAbleToRemove()
end	
function c11113000.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return false end
		local mg=Duel.GetMatchingGroup(c11113000.matfilter,tp,LOCATION_HAND+LOCATION_GRAVE,0,nil)
		return Duel.IsExistingMatchingCard(c11113000.filter,tp,LOCATION_HAND,0,1,nil,e,tp,mg)
	end
...
function c11113000.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.SelectMatchingCard(tp,c11113000.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp,mg)
...
function c11113000.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c11113000.thfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c11113000.thfilter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectTarget(tp,c11113000.thfilter,tp,LOCATION_REMOVED,0,1,1,nil)
