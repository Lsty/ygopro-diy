...
function c114100066.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	local p2=false
	while tc do
		if not tc:IsSetCard(0x221) then --( tc:IsRace(RACE_WARRIOR) and tc:IsAttribute(ATTRIBUTE_DARK) )
...
function c114100066.splimit(e,c,sump,sumtype,sumpos,targetp,se)
	return not c:IsSetCard(0x221) and e:GetLabelObject()~=se --not ( c:IsRace(RACE_WARRIOR) and c:IsAttribute(ATTRIBUTE_DARK) )
end
--end of cost
